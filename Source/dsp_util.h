...
inline auto checkNan(const float a){
    return a != a;  // NOLINT this is a well-known way to check for NaN
}
inline auto checkInf(const float a){
    return std::isinf(a);
}
inline bool checkNanOrInf(const std::span<float> sp) {
    for (auto const &a : sp){
...
inline float scale(const float val, const float min, const float range){
...
inline void calculateSymmetricEnvelope(const juce::dsp::AudioBlock<float>& audioBlock,
    ...
    const size_t numSamples = audioBlock.getNumSamples();
